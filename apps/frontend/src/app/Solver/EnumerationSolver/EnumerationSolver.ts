...
export class EnumerationSolver extends SolverBase<WorkerCommand, WorkerResult> {
  ...
  protected startWorkers(): void {
    ...
    const countCommand: WorkerCommand = {
      command: 'count',
      exclusion: this.artSetExcl,
      arts: [this.arts],
    };
  ...
  constructor(input: OptProblemInput) {
    ...
    const setPerms = filterFeasiblePerm(
      artSetPerm(
        this.artSetExcl,
        Object.values(this.arts.values).flatMap((x) => x.map((x) => x.set!))
      ),
      this.arts
    );
