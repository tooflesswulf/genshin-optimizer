...
export default function TabBuild() {
  ...
  const [{ equipmentPriority, threads = defThreads }, setDisplayOptimize] =
    useState(database.displayOptimize.get());
  useEffect(
    () => database.displayOptimize.follow((r, to) => setDisplayOptimize(to)),
    [database, setDisplayOptimize]
  );

  const maxWorkers = threads > defThreads ? defThreads : threads;
  const setMaxWorkers = useCallback(
    (threads) => database.displayOptimize.set({ threads }),
    [database]
  );
  ...
  const filteredArts = useMemo(() => {
    ...
    if (useEquippedArts) {
      ...
      if (index < 0)
        equipmentPriority.forEach((ek) =>
          cantTakeList.add(charKeyToLocCharKey(ek))
        );
      else
        equipmentPriority
          .slice(0, index)
          .forEach((ek) => cantTakeList.add(charKeyToLocCharKey(ek)));
    }
    return database.arts.values.filter((art) => {
      ...
      if (art.location && useEquippedArts && cantTakeList.has(art.location))
        return false;
      return true;
    });
  }, [
  ...
  const generateBuilds = useCallback(async () => {
    ...
    let arts = split!;
    const setPerms = filterFeasiblePerm(
      artSetPerm(
        artSetExclusion,
        Object.values(split.values).flatMap((x) => x.map((x) => x.set!))
      ),
      split
    );
    ...
    // const wrap = {
    //   buildValues: Array(maxBuildsToShow)
    //     .fill(0)
    ...
    setWorkerErr(false);
    const solver: SolverBase<unknown, { command: string }> =
      new EnumerationSolver(setup2);
    ...
    const buildTimer = setInterval(
      () => setBuildStatus({ type: 'active', ...solver.computeStatus }),
      100
    );
                ...
                <CardContent>
                  <Box display="flex" alignItems="center" gap={1}>
                    <AssumeFullLevelToggle
                      ...
                      setmainStatAssumptionLevel={(mainStatAssumptionLevel) =>
...
function DataContextWrapper({ children, characterKey, build, oldData }: Prop) {
  ...
  const buildsArts = useMemo(
    () => dirty && build.map((i) => database.arts.get(i)!),
