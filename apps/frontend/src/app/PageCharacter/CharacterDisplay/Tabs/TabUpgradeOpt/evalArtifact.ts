...
export function evalArtifact(objective: Query, art: QueryArtifact, slow = false, calc4th = false): QueryResult {
  ...
  const scale = (key: SubstatKey) => key.endsWith('_') ? Artifact.substatValue(key, art.rarity) / 1000 : Artifact.substatValue(key, art.rarity) / 10
...
function fastGaussian({ rollsLeft, subs, stats, thresholds, calc4th, scale, objectiveEval }: InternalQuery): InternalResult {
...
function slowGMMnd({ rollsLeft, stats, subs, thresholds, calc4th, scale, objectiveEval }: InternalQuery): InternalResult {
  ...
  crawlUpgrades(rollsLeft, (ns, p) => {
    ...
    const cov = obj.map(o1 => obj.map(o2 => o1.ks.reduce((pv, cv, k) => pv + o1.ks[k] * o2.ks[k] * ns[k], 0)))
