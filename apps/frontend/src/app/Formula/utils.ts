...
export function deepCmpNode(a: NumNode, b: NumNode): boolean {
  ...
  switch (a.operation) {
    ...
    case 'max': {
      ...
      // Check every pair of orderings
      return a.operands.every((ai, i) =>
        b.operands.some((bj, j) => {
          if (used.includes(j)) return false
          if (deepCmpNode(ai, bj)) {
            used.push(j)
            return true
          }
          return false
    ...
    default:
      return a.operands.every((ai, i) =>
        deepCmpNode(ai as NumNode, b.operands[i] as NumNode)
      )
  }
}
...
export function hashNode(n: NumNode): number {
  const hsh = Math.imul(cyrb53(n.operation), 5234543537)
  switch (n.operation) {
    case 'const':
    ...
    case 'add':
    case 'min':
    case 'max': // commutative ops.
      return n.operands.reduce((hsh, ni) => hsh ^ hashNode(ni), hsh)
    default:
      return (n.operands as NumNode[]).reduce(
        (hsh, ni) => Math.imul(hsh ^ hashNode(ni), 9923429423),
        hsh
      )
  }
}
